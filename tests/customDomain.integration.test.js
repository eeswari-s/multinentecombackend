...
describe('Custom domain management', () => {
  ...
  test('verification fails while the TXT record is missing', async () => {
    ...
    const res = await owner(request(app).post('/api/v1/client-admin/store-settings/custom-domain/verify'));
  ...
  test('verification succeeds once the TXT record matches, and the domain becomes usable for tenant resolution', async () => {
    ...
    const statusRes = await owner(request(app).get('/api/v1/client-admin/store-settings/custom-domain'));
  ...
  test('removing the domain clears it and it no longer resolves', async () => {
    const res = await owner(request(app).delete('/api/v1/client-admin/store-settings/custom-domain'));
    expect(res.status).toBe(200);
